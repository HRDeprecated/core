...
Builder.prototype.genHTML = function() {
    ...
    if (_.isEmpty(this.config.index)) {
        fs.writeFileSync(path.resolve(this.config.build, "index.html"), utils.parse(INDEX_HTML_TEMPLATE, this.config.name, this.config.out.css, this.config.out.js));
    } else {
        utils.copyFile(this.config.index, path.resolve(this.config.build, "index.html"));
    }
...
Builder.prototype.genRJSConfig = function() {
    ...
    return {
        ...
        optimize: config.debug ? "uglify" : "none"
    };
};
